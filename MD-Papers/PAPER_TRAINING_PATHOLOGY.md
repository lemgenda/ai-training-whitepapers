...
| **Windows Multiprocessing Heap Duplication & Pagefile Thrashing** | PyTorch DataLoader with `num_workers > 0` on Windows uses `spawn`, duplicating large memory-mapped sequence datasets across child processes. 4 workers consume 20+ GB, driving committed memory to 53GB / 55.5GB and exhausting physical RAM to 1.3GB. | GPU utilization collapses to 0%; batch retrieval latency balloons from 0.27s to 3.43s/batch; committed memory exceeds 95%; task manager shows 4 child workers holding 5GB RAM each. | Enforce **Zero-Worker Execution (`num_workers=0`)** for memory-mapped sequence datasets on Windows. In-process execution operates at 1.1GB RAM total, preserving 20+ GB free RAM for OS filesystem caching and accelerating batch retrieval by $12.5\times$. |
| **Checkpoint Resumption Fraction Desynchronization (Ghost Loader Dilution)** | Resuming training from an earlier checkpoint saved prior to curriculum fractional scaling (where `sample_fraction` was 1.0) overrides the active curriculum, expanding batch count by 7x (3,900 to 27,300 batches) and diluting convergence rate. | Checkpoint telemetry reports `Source Fraction: 100.0%`, `New Length: 27300` (instead of ~3,900), and epoch progress bar indicates 24–26+ hours remaining for Epoch 1. | Implement **Curriculum Checkpoint Fraction Override** in `SmartTrainingGovernor.load_state`. For sequence curricula during early epochs ($\le 2$), override legacy $\ge 0.99$ fractions with configured `initial_fraction: 0.15` and synchronize DataLoader instances. |
...
### Windows Multiprocessing Heap Duplication & Pagefile Thrashing (Pathology 24)

- **The Issue**: On Windows platforms, PyTorch `DataLoader` with `num_workers > 0` cannot leverage UNIX `fork()` with Copy-On-Write memory semantics; it must use `multiprocessing.spawn`. When training on extensive multi-timeframe sequence manifolds (e.g. 16-symbol Forex arrays across 8 years), each child worker process unpickles the dataset sample index and establishes isolated file descriptors for memory-mapped `.npy` files. Each worker allocates 4.5–5.5 GB of RAM. Spawning 4 workers consumes 20+ GB of physical RAM, driving committed memory to 53.0 GB / 55.5 GB on a 32 GB workstation. Available RAM collapses to 1.3 GB, forcing Windows into continuous pagefile disk thrashing. Every sample read triggers disk I/O page faults, inflating batch latency from 0.27s to 3.43s and starving the GPU (0% utilization).
- **Identification**: Task Manager displays 4 `python.exe` worker processes holding ~5 GB RAM each; committed memory approaches the pagefile limit (> 95%); batch retrieval time stalls at 3.4–4.0s/batch; and GPU utilization drops to 0%.
- **Remedy**: **Zero-Worker Windows In-Process Execution (`num_workers=0`)**. Enforce `num_workers = 0` on Windows for memory-mapped sequence datasets. Execution runs directly in the parent process at 1.1 GB total RAM, leaving 20+ GB of physical memory free. The Windows filesystem cache retains `.npy` array blocks resident in RAM, eliminating pagefile swapping and accelerating batch retrieval by $12.5\times$ (0.275s/batch).

### Checkpoint Resumption Fraction Desynchronization (Ghost Loader Dilution) (Pathology 25)

- **The Issue**: When `SmartTrainingGovernor` saves progress checkpoints (`_progress.pth`), it serializes the active `sample_fraction`. If a run is interrupted and resumed from an older checkpoint generated before fractional curriculum fixes (where the sample fraction was hardcoded to 1.0), the checkpoint restoration logic blindly restores `current_fraction = 1.0`. This expands the training dataloader from the intended curriculum fraction (e.g. 15%, ~3,900 batches) to 100% of the dataset (27,300 batches), multiplying epoch training time by $7\times$ (26+ hours) and diluting early-epoch learning rates across 7 million samples.
- **Identification**: Resumption telemetry logs report `Source Fraction: 100.0%`, `New Length: 27300`, and `Iteration 1362/27300` despite configuration specifying `initial_fraction: 0.15`.
- **Remedy**: **Curriculum Checkpoint Fraction Override**. In `SmartTrainingGovernor.load_state`, detect whether early-epoch training ($\text{epoch} \le 2$) is resuming with a legacy 100% fraction against an active fractional curriculum. Automatically override the legacy fraction with `initial_fraction: 0.15`, trigger `train_loader` re-instantiation, and proportionally scale resumption iterations to the actual loader length.
